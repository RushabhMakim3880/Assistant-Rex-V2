...
# Globalfrom mobile_bridge import MobileBridge
...
DEFAULT_SETTINGS = {
    ...
    "master_control": False # Bypass all permissions if True
}
...
async def lifespan(app: FastAPI):
    ...
    # Auto initialization of Jarvis in Background
    print("[SERVER] Startup: Initializing REX in background...")
    asyncio.create_task(init_jarvis())

    yield
    # --- Shutdown Logic ---
    print("[SERVER DEBUG] Lifespan Shutdown Triggered")
    # Clean up resources if needed (most are handled by signal handlers currently)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Allow all origins for local dev (Mobile App + React)
...
@app.get("/status")
async def status():
    return {"status": "running", "message": "Backend is ready!"}
...
async def get_network_info():
    ...
    try:
        # Best effort to find actual LAN IP
    ...
    return {"ip": ip_address, "port": 8000, "url": f"http://{ip_address}:8000"}
...
def cb_on_mobile_command(event, args=None):
    # event is already fully qualified (e.g., 'mobile:open_app')
...
async def init_jarvis(device_index=None, device_name=None, muted=False):
    ...
    try:
        ...
        audio_loop = jarvis.AudioLoop(
            video_mode="none", 
            on_audio_data=cb_on_audio_data,
            on_cad_data=cb_broadcast_cad_data,
            on_web_data=cb_broadcast_web_data,
            on_transcription=cb_broadcast_transcription,
            on_tool_confirmation=cb_on_tool_confirmation,
            on_cad_status=cb_on_cad_status,
            on_cad_thought=cb_on_cad_thought,
            on_project_update=cb_on_project_update,
        ...
        # Apply master control if set
        if SETTINGS.get("master_control"):
             audio_loop.set_master_control(True)
...
async def start_audio(sid, data=None):
    ...
    print("Starting Audio Loop requested by client...")
    ...
    await init_jarvis(device_index, device_name, muted)

    try:
        print(f"Initializing AudioLoop with device_index={device_index}")
        # Re-define callbacks here ensuring they capture current scope or use globals if needed
        # Since we moved this out of start_audio, we need to ensure the callbacks (on_audio_data etc) are accessible.
        # They were inner functions of start_audio. We need to move them to global scope or class.
        
        # FOR CODE SAFETY: I will rely on the global callbacks defined below or move them out.
        # Actually, extracting them is messy with 'replace_file_content'.
        # better strategy: keep start_audio as is, but make it callable internally.
        pass 
    except Exception as e:
        print(f"Error init jarvis: {e}")

# RE-STRATEGY: The callbacks are inside start_audio. I needs to move them out to make this clean.
# But for now, I will just make lifespan call a new setup function, and move callbacks to global scope.
...
@sio.on('mobile:connect')
async def on_mobile_connect(sid, data):
    print(f"[SERVER] Mobile Device Connected: {sid} Data: {data}")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.connect_device(data.get('device_id', sid))
        await sio.emit('mobile:status', {'msg': 'Connected to REX Brain'}, room=sid)

@sio.on('mobile:call_state')
async def on_mobile_call_state(sid, data):
    # data: { 'state': 'RINGING', 'number': ... }
    print(f"[SERVER] Received Mobile Call State: {data}")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.receive_call_state(data)

@sio.on('mobile:notification')
async def on_mobile_notification(sid, data):
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.receive_notification(data)

@sio.on('mobile:contact_results')
async def on_mobile_contact_results(sid, data):
    print(f"[SERVER] Received Mobile Contact Results: {data}")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.handle_contact_results(data)

@sio.on('mobile:audio_stream')
async def on_mobile_audio_stream(sid, data):
    loop = asyncio.get_running_loop()
    def send_to_mobile(data):
         asyncio.run_coroutine_threadsafe(sio.emit('mobile:audio_out', list(data)), loop)
    
    def send_command(event, data):
         asyncio.run_coroutine_threadsafe(sio.emit(event, data), loop)

    audio_loop.mobile_bridge.set_audio_output_handler(send_to_mobile)
    audio_loop.mobile_bridge.set_command_handler(send_command)

    # Start the loop
    loop.create_task(audio_loop.run())
    
    # Start System Monitor
    global system_monitor
    system_monitor = SystemMonitor(sio)
    loop.create_task(system_monitor.start())
...
@sio.on('mobile:connect')
async def handle_mobile_connect(sid, data):
    print(f"[SERVER] Mobile Device Connected: {data}")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.connect_device(data.get('device_id', 'unknown'))
    await sio.emit('mobile:status', {'status': 'connected'})

@sio.on('disconnect')
async def handle_disconnect(sid):
    print(f"[SERVER] Client Disconnected: {sid}")
    # We might want to track if it was the mobile device
    # For now, safe to tell bridge if we knew it was them, but we don't track SID mapping yet.
    # audio_loop.mobile_bridge.disconnect_device() 
    pass

@sio.on('mobile:call_state')
async def handle_mobile_call_state(sid, data):
    # data: { state, number }
    print(f"[SERVER] Mobile Call State: {data}")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.receive_call_state(data)

@sio.on('mobile:audio_stream')
async def handle_mobile_audio(sid, data):
    # Data is raw PCM bytes (uint8 or int16)
    if audio_loop and audio_loop.mobile_bridge:
        # We need to make sure format matches what pyaudio expects
        # Flutter sends Uint8List, which might be byte array in python
        audio_loop.mobile_bridge.receive_audio(data)

@sio.on('mobile:contact_results')
async def handle_mobile_contact_results(sid, data):
    print(f"[SERVER] Mobile Contact Results: {len(data.get('results', []))} found")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.handle_contact_results(data)

@sio.on('mobile:contact_list')
async def handle_mobile_contact_list(sid, data):
    print(f"[SERVER] Mobile Full Contact List: {len(data.get('results', []))} items")
    if audio_loop and audio_loop.mobile_bridge:
        # We can reuse the same handler, but maybe add a flag for full list
        data['is_full_sync'] = True
        audio_loop.mobile_bridge.handle_contact_results(data)

@sio.on('mobile:location_results')
async def handle_mobile_location_results(sid, data):
    print(f"[SERVER] Mobile Location Results: {data}")
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.handle_location_results(data)
...
@sio.on('mobile:file_beam_response')
async def handle_mobile_file_beam_response(sid, data):
    if audio_loop and audio_loop.mobile_bridge:
        audio_loop.mobile_bridge.handle_file_beam_response(data)

@sio.on('mobile:test_audio')
async def handle_mobile_test_audio(sid):
    print("Received Audio Test Request")
    return "OK"
...
async def update_settings(sid, data):
    ...
    if 'master_control' in data:
        new_mc = data['master_control']
        SETTINGS['master_control'] = new_mc
        print(f"Updating Master Control: {new_mc}")
        if audio_loop:
            audio_loop.set_master_control(new_mc)
        save_settings()
        await sio.emit('settings', SETTINGS)
