    ...
    return (
        <div className="absolute top-20 right-10 bg-black/90 border border-cyan-500/50 p-4 rounded-lg z-50 w-80 backdrop-blur-xl shadow-[0_0_30px_rgba(6,182,212,0.2)]">
            ...
            <div className="mb-6">
                ...
                <div className="flex items-center justify-between text-xs bg-red-900/30 p-2 rounded border border-red-500/50 mb-3">
                    <span className="text-red-200 font-bold">MASTER CONTROL (Bypass All)</span>
                    <button
                        onClick={toggleMasterControl}
                        className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${masterControlEnabled ? 'bg-red-500' : 'bg-gray-700'}`}
                    >
                        <div
                            className={`absolute top-0.5 left-0.5 w-3 h-3 bg-white rounded-full transition-transform duration-200 ${masterControlEnabled ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                    </button>
                </div>

                <div className={`space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar ${masterControlEnabled ? 'opacity-50 pointer-events-none' : ''}`}>
                    {TOOLS.map(tool => {
